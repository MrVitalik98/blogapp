...
import { FaPen, FaPencilAlt, FaTimes, FaPlus, FaTrash, FaCheck } from 'react-icons/fa'
import { UPLOAD_AVATAR, EDIT_USER_DATA, DELETE_AVATAR } from '../graphql/user/mutations'
...
const Profile = () => {
  ...
  const { user: { fullname, avatar, about, _id } } = useSelector(state => state.auth)
  ...
  const [uploadAvatar, { loading: loading1, data: data1 }] = useMutation(UPLOAD_AVATAR, {
    refetchQueries
  })

  const [deleteAvatar, { loading: loading2, data: data2 }] = useMutation(DELETE_AVATAR, {
    refetchQueries
  })

  const [editUserData, { loading: loading3, data: data3 }] = useMutation(EDIT_USER_DATA, {
    refetchQueries
  })
  ...
  useEffect(() => {
    ...
    (data1?.result?.status === 'error' || data2?.result?.status === 'error') && dispatch(showAlert(data1?.result || data2?.result))
  }, [loading1, loading2, data1, data2, dispatch])


  useEffect(() => {
    data3?.result && data3?.result?.status === 'success' && !loading3 && setActiveTextArea(false)
  }, [loading3, data3])
      ...
      <div className="user-avatar">
        ...
        {file && (
          ...
          >
            <button
              ...
              onClick={() => !loading1 && isActiveEditAvatarBtn ? setFile('') : undefined}
            ...
            <button
              ...
              id="upload-avatar"
              onClick={!loading1 && isActiveEditAvatarBtn ? handleUploadAvatar : undefined}
              disabled={loading1 || !isActiveEditAvatarBtn}
            >
              <FaCheck />
            </button>
        ...
        {!file && 
          <>
            <button
              id="select-avatar"
              className={`btn btn-secondary shadow ${isActiveEditAvatarBtn && 'show'}`}
              onClick={() => isActiveEditAvatarBtn && !loading2 ? fileInputRef.current.click() : undefined}
            ...
            <button
              id="remove-avatar"
              className={`btn btn-danger shadow ${isActiveEditAvatarBtn && 'show'}`}
              onClick={() => avatar && isActiveEditAvatarBtn && !loading2 ? dispatch(openDeleteAvatarModal()) : undefined}
            ...
            {isActiveEditAvatarBtn 
              ? (
                  <button
                    ...
                    id="edit-avatar"
                    onClick={() => loading2 ? undefined : setActiveEditAvatarBtn(false)}
                  >
                    <FaTimes />
                  </button>
                )
              : (
                  <button
                    ...
                    id="edit-avatar"
                    onClick={() => loading2 ? undefined : setActiveEditAvatarBtn(true)}
                  >
                    <FaPencilAlt />
                  </button>
                )
            ...
            ? <>
                <div className="form">
                  <textarea 
                    ...
                    maxLength={200}
                    value={aboutMe}
                    onChange={e => !loading3 ? setAboutMe(e.target.value) : undefined}
                    disabled={loading3}
                  ></textarea>
                ...
                <div className={`btn-group ${!about ? 'isEmpty' : ''}`}>
                  {about && <button 
                      ...
                      onClick={() => !loading3 ? setActiveTextArea(false) : undefined}
                      disabled={loading3}
                    >
                      Cancel
                    </button>
